 ...
 * June 2003
 *
 * assembly.h - assembly language functions and prototypes for supported
 * platforms
 *
 ...
 *
 * MULSHIFT32(x, y)    signed multiply of two 32-bit integers (x and y), returns
 * top 32 bits of 64-bit result FASTABS(x)          branchless absolute value of
 * signed integer x CLZ(x)              count leading zeros in x MADD64(sum, x,
 * y)   (Windows only) sum [64-bit] += x [32-bit] * y [32-bit] SHL64(sum, x, y)
 * (Windows only) 64-bit left shift using __int64 SAR64(sum, x, y)    (Windows
 * only) 64-bit right shift using __int64
 */
...
extern "C" {
    ...
    (defined __WINS__ && defined _SYMBIAN) || defined(_OPENWAVE_SIMULATOR) ||  \
    defined(WINCE_EMULATOR) /* Symbian emulator for Ix86 */

#pragma warning(                                                               \
    disable : 4035) /* complains about inline asm not returning a value */
...
/* use asm function for now (EVC++ 3.0 does horrible job compiling __int64
 * version) */
...
__attribute__((__always_inline__)) static __inline short SAR64_Clip(Word64 x) {
  ...
  int xHi = (int)(x >> 32);
  __asm__ __volatile__("lsr %1, %1, #26\n\t"        // xLo <- xLo>>n
                       "orr %1, %1, %0, lsl #6\n\t" // xLo <= xLo || (xHi << 6)
                       : "+&r"(xHi), "+r"(xLo));
  return ((short)xLo);
}

__attribute__((__always_inline__)) static __inline Word64 SAR64(Word64 x,
                                                                int n) {
  unsigned int xLo = (unsigned int)x;
  int xHi = (int)(x >> 32);
  int nComp = 32 - n;
...
__attribute__((__always_inline__)) static __inline int MULSHIFT32(int x,
                                                                  int y) {
...
__attribute__((__always_inline__)) static __inline int FASTABS(int x) {
  int tmp;
  __asm__ __volatile__("abs %0" : "=r"(tmp) : "r"(x));
  return tmp;
}

__attribute__((__always_inline__)) static __inline int CLZ(int x) {
  int tmp;
  __asm__ __volatile__("clz %0,%1" : "=r"(tmp) : "r"(x));
  return tmp;
}
 ...
 */
__attribute__((__always_inline__)) static __inline Word64 MADD64(Word64 sum,
                                                                 int x, int y) {
  __asm__ __volatile__("macs.d %0, %1, %2" : "+r"(sum) : "r"(x), "r"(y));
  return (sum);
}
...
static inline int MULSHIFT32(int x, int y) { return ((int64_t)x * y) >> 32; }
...
static inline int CLZ(int x) { return __builtin_clz(x); }
...
static inline short SAR64_Clip(Word64 x) { return SAR64(x, 26); }
